...
class IPDBv4(object):
	...
	def getIPAddr(self, ip):
		...
		try:
			...
			# 得到索引记录
			ip_off = self.firstIndex + i * (4 + self.offlen)
			ip_rec_off = self.getLong8(ip_off + 4, self.offlen)
			(c, a) = self.getAddr(ip_rec_off)
			(cc, aa) = (c, a)
			i1 = inet_ntoa(self.getLong4(ip_off))
			try:
				i2 = inet_ntoa(self.getLong4(ip_off + 4 + self.offlen) - 1)
			except:
				i2 = "255.255.255.255"
		except:
			i1 = ""
